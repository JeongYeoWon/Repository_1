...
var rloginBtn = document.getElementById('rloginBtn');
// 로그아웃 버튼
...
rloginBtn.addEventListener('click', async function() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      <form action="2048-1.html"><button id="logout" style = 'display : block;'>로그아웃</button></form>
      `;
    } else {
      // 요청이 실패한 경우
...
// 랭킹 플레이어 이름 변경
async function playerRankingData(){
  ...
  try {
    ...
    userRank[5].textContent = data.username;
  } catch (error) {
